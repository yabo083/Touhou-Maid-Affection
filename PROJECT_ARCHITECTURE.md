...
- `LapPillowState` 保存本次会话的女仆 UUID、锚点 UUID、姿态快照以及女仆原始坐/睡状态。
...
- 玩家处于 lying 分支时，会话不再强制维持“玩家骑乘锚点”；锚点保留为坐标中间者，玩家改为服务端按锚点偏移持续定位并保持睡姿，从而避开骑乘坐姿对躺姿的覆盖。
...
- 这是“客户端热键发起，服务端会话维持”的姿态状态机，但实际空间基准不再直接依赖玩家传送，而是依赖中间锚点实体。
...
- 玩家 lying 分支当前仍以 `forced pose + 客户端睡姿桥接` 为主，未正式接入 fake-bed 睡眠链路；此前对 `sleepingPos` 的试探性接入已回退，因为它会把玩家拖入原版真正睡眠流程并引入黑屏与坐立抖动。当前的探索方向是继续通过客户端渲染桥接补齐“只躺不睡”的视觉表现，而不是重新走原版睡眠状态机。
- 当玩家处于 lying 分支时，中间锚点会关闭 `shouldRiderSit`，以避免乘骑坐姿覆盖躺姿渲染链路。
