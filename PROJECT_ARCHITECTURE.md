...
羁绊页（TLM 顶部页签进入的主页，即选项卡界面）与它弹出的二级页的**底板**改为图片渲染，与设置面板同机制：所有原本用 `GuiGraphics#fill` 画出来的面板、边框与分隔线都烘进可替换贴图，代码不再自绘底色与边框。本次范围**只含底板**——按钮、下拉框、滑块、开关、数值框、列表行与全部文字仍由代码绘制。

- `textures/gui/bond_page.png`（528×411 RGBA = 页面 176×137 逻辑尺寸 × 3）：外框与斜面、标题条、表头/表尾/左右分隔线、内凹内容区一次烘入。`BondMaidContainerScreen#renderBondPageBackground` 只做一次 1:1 blit（GUI scale 3 下不缩放、不额外着色），随后仍由代码绘制居中的本地化标题 `bond.tab.title`。
- `textures/gui/bond_row.png`（474×69 = 能力行 158×23 × 3；行距是 24，但 `BondAbilityRowLayout` 的填充盒是 `rowTop..rowTop+23`）：能力行底板，替换原先的纯色 `fill`，因此必须按 23 逻辑像素而不是 24 烘焙，否则会被重采样。
- `textures/gui/bond_modal.png`（42×90，九宫格切片 6|2|6 × 22|2|6 逻辑像素）：二级页模态框的外框、面板与表头分隔线。表头上沿切片固定为 `MODAL_TITLE_HEIGHT + 1`（标题带 + 分隔线一行），因此 172×150（分栏页）与 166×132（语音页）以及未来任意尺寸共用同一张图；模态框的压暗层仍是代码 `fill`。
- `textures/gui/bond_inset.png`（24×24，九宫格切片 3|2|3 逻辑像素）：二级页内部的内凹底板，覆盖 `BondScrollableList` 的列表框（语音池列表、救护动作列表）、语音页空列表占位、膝枕页左面板与姿态网格。
- 切片几何抽在纯逻辑类 `BondNinePatch`（源切片以贴图像素给、目标以 GUI 像素给，按 `TEXTURE_SCALE = 3` 换算），`BondNinePatchTest` 断言四角保持原始尺寸、中心吸收全部余量、被压扁到小于边框时丢弃空切片而不是产生负矩形。贴图入口统一在 `BondGuiArt`；资源包替换成其他尺寸时按目标矩形拉伸，行为与 `settings_panel.png` 相同。

四张贴图目前是**原版风格占位图**（黑描边、上/左亮斜面、下/右暗斜面、内凹槽位），手工绘制后直接替换同名文件即可，不需要改代码；由于文字与控件未动，换底图时只需保证承载文字的区域对比度足够。本次改动不移动任何布局、命中测试或滚动代码。
...
- 新增 GUI 背景/边框优先走可替换贴图（与 `settings_panel.png`、`bond_page.png` 同机制：按 3 倍逻辑尺寸烘焙、1:1 blit），不要让 chrome 退回硬编码 `fill`；控件内部细节仍由代码绘制。
