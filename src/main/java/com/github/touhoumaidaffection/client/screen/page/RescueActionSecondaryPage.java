        ...
        if (!buttonId.isEmpty()) {
            switch (buttonId) {
                case "save" -> {
                    ...
                    TouhouMaidAffection.CHANNEL.sendToServer(new RescueActionConfigPayload(host.getMaid().getUUID(), selectedActionId));
                    host.closeSecondaryPage();
                }
                case "clear" -> {
                    selectedActionId = "";
                    RescueYsmActionConfig.setSelectedAction(getModelId(), getTextureId(), "");
                    TouhouMaidAffection.CHANNEL.sendToServer(new RescueActionConfigPayload(host.getMaid().getUUID(), ""));
                    host.closeSecondaryPage();
                }
