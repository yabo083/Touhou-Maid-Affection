...
public final class LapPillowPoseSecondaryPage implements BondSecondaryPage {
    private static final int MODAL_WIDTH = 190;
    private static final int MODAL_HEIGHT = 132;
    private static final int GRID_SIZE = 82;
    ...
    private static final int COORD_CARD_WIDTH = 74;
    private static final int COORD_CARD_HEIGHT = 14;
    ...
    public LapPillowPoseSecondaryPage(BondSecondaryPageHost host) {
        ...
        this.maidActionDropdown = new BondDropdown<>(modal.left() + 106, modal.top() + 34, DROPDOWN_WIDTH, DROPDOWN_HEIGHT, DROPDOWN_ROW_HEIGHT, DROPDOWN_VISIBLE_ROWS);
        this.playerActionDropdown = new BondDropdown<>(modal.left() + 106, modal.top() + 58, DROPDOWN_WIDTH, DROPDOWN_HEIGHT, DROPDOWN_ROW_HEIGHT, DROPDOWN_VISIBLE_ROWS);
    ...
    private void drawGridPanel(GuiGraphics graphics, Font font, BondModalPage modal) {
        int panelLeft = modal.left() + 10;
        int panelTop = modal.top() + 18;
        int panelRight = panelLeft + 88;
        int panelBottom = panelTop + 98;
        graphics.fill(panelLeft, panelTop, panelRight, panelBottom, 0xFF252525);
        graphics.fill(panelLeft + 1, panelTop + 1, panelRight - 1, panelBottom - 1, 0xFF151515);

        graphics.drawString(font, Component.translatable("bond.lap_pillow.relative_position"), panelLeft + 4, panelTop + 4, 0xFFE0E0E0, false);

        int gridLeft = panelLeft + 4;
        int gridTop = panelTop + 14;
        int gridRight = gridLeft + GRID_SIZE;
        int gridBottom = gridTop + GRID_SIZE;
        graphics.fill(gridLeft, gridTop, gridRight, gridBottom, 0xFF202020);
        ...
        drawPoint(graphics, centerX, centerY, workingPose.maidOffsetX(), workingPose.maidOffsetZ(), 0xFFFF6B6B, activePoint == EditablePoint.MAID);
        drawPoint(graphics, centerX, centerY, workingPose.playerOffsetX(), workingPose.playerOffsetZ(), 0xFFF5F5F5, activePoint == EditablePoint.PLAYER);
    }

    private void drawRightPanel(GuiGraphics graphics, Font font, BondModalPage modal, int mouseX, int mouseY) {
        int left = modal.left() + 106;
        int top = modal.top() + 18;

        graphics.drawString(font, Component.translatable("bond.lap_pillow.maid_action"), left, top + 7, 0xFFE0E0E0, false);
        ...
        graphics.drawString(font, Component.translatable("bond.lap_pillow.player_action"), left, top + 31, 0xFFE0E0E0, false);
        ...
        drawCoordCard(graphics, font, left, top + 55, COORD_CARD_WIDTH, Component.translatable("bond.lap_pillow.subject.player"), workingPose.playerOffsetX(), workingPose.playerOffsetY(), workingPose.playerOffsetZ(), activePoint == EditablePoint.PLAYER);
        drawCoordCard(graphics, font, left, top + 73, COORD_CARD_WIDTH, Component.translatable("bond.lap_pillow.subject.maid"), workingPose.maidOffsetX(), workingPose.maidOffsetY(), workingPose.maidOffsetZ(), activePoint == EditablePoint.MAID);
    }
    ...
    private void renderActionOption(GuiGraphics graphics, Font font, ActionOption option, int index, int left, int top, int right, int height, boolean hovered, boolean selectedHeader, boolean selected) {
        ...
        graphics.drawString(font, option.label(), left, top + 1, selected ? 0xFFFFE08A : 0xFFEAEAEA, false);
    }
    ...
    private void drawCoordCard(GuiGraphics graphics, Font font, int left, int top, int width, Component title, double x, double y, double z, boolean active) {
        graphics.fill(left, top, left + width, top + COORD_CARD_HEIGHT, active ? 0xFF3B3232 : 0xFF2B2B2B);
        graphics.fill(left + 1, top + 1, left + width - 1, top + COORD_CARD_HEIGHT - 1, 0xFF161616);
        graphics.pose().pushPose();
        try {
            graphics.pose().translate(left + 3.0F, top + 3.0F, 0.0F);
            graphics.pose().scale(0.70F, 0.70F, 1.0F);
            graphics.drawString(font, Component.translatable("bond.lap_pillow.coords.compact", title.getString(), formatValue(x), formatValue(y), formatValue(z)), 0, 0, 0xFFE8E8E8, false);
        } finally {
    ...
    private PointChoice detectPointChoice(double mouseX, double mouseY, BondModalPage modal) {
        ...
        int gridLeft = modal.left() + 14;
        int gridTop = modal.top() + 32;
        int centerX = gridLeft + GRID_SIZE / 2;
        int centerY = gridTop + GRID_SIZE / 2;
    ...
    private void saveAndClose() {
        ...
        PacketDistributor.sendToServer(new LapPillowPoseConfigPayload(
                maidUuid,
    ...
    private List<BondButtonRow.ButtonSpec> buttons(BondModalPage modal) {
        return BondButtonRow.createCentered(modal.width(), modal.bottom() - 22, BUTTON_WIDTH, BUTTON_HEIGHT, BUTTON_GAP,
                new BondButtonRow.ButtonSpec(0, 0, BUTTON_WIDTH, BUTTON_HEIGHT, Component.translatable("bond.lap_pillow.save"), "save", true),
                new BondButtonRow.ButtonSpec(0, 0, BUTTON_WIDTH, BUTTON_HEIGHT, Component.translatable("bond.lap_pillow.reset"), "reset", true),
