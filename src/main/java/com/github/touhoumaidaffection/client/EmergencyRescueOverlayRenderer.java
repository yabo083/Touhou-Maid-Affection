...
public final class EmergencyRescueOverlayRenderer {
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        float progress = activeOverlay.progress(event.getPartialTick().getGameTimeDeltaPartialTick(false));
        ...
        try {
            ...
            InventoryScreen.renderEntityInInventory(
                    gui,
                    translatedCenterX,
                    translatedCenterY + scale * 0.95f,
                    scale,
                    new Vector3f(0.0f, activeOverlay.maid().getBbHeight() * 0.5f, 0.0f),
                    bodyRotation,
                    cameraRotation,
                    activeOverlay.maid()
            );
    ...
    private static void tickOverlayMaid(ActiveOverlay overlay) {
        ...
        if (maid.isYsmModel()) {
            ...
            NeoForge.EVENT_BUS.post(new YsmMaidClientTickEvent(maid));
        }
    }
    ...
    private static void syncYsmState(EntityMaid maid) {
        ...
        NeoForge.EVENT_BUS.post(new UpdateRemoteStructEvent(maid, new Object2FloatOpenHashMap<>(maid.roamingVars)));
    }
