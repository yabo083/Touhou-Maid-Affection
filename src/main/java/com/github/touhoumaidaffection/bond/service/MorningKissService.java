...
public final class MorningKissService {
    ...
    public static String getAllowedTimeRangesText() {
        return MorningKissScheduleRules.formatAllowedTimeRanges(ModConfig.BOND_MORNING_KISS_ALLOWED_TIME_RANGES.get());
    }
    ...
    private static void scheduleOrStartAutoTasks(ServerPlayer player, ActiveTimeWindow activeWindow) {
        ...
        for (EntityMaid maid : nearbyMaids) {
            ...
            if (start(player, maid, TriggerSource.AUTO_WINDOW, activeWindow.windowId(), false)) {
                MorningKissDialogueService.showMessage(player, MorningKissDialogueService.startMessage(
                        "bond.morning_kiss.auto_started", "bond.morning_kiss.auto_started.chat", maid));
            } else {
    ...
    private static void tickActiveTasks(MinecraftServer server) {
        ...
        while (iterator.hasNext()) {
            ...
            maid.getNavigation().stop();
            MorningKissVoiceService.Selection selectedVoice = MorningKissVoiceService.select(player, maid);
            boolean playKissSound = MorningKissVoiceService.shouldPlayKissSound(player, maid, selectedVoice);
    ...
    private static boolean start(Player player, EntityMaid maid, TriggerSource triggerSource, String autoWindowId, boolean showStartMessage) {
        ...
        if (failureKey != null) {
            if (showStartMessage) {
                MorningKissDialogueService.showMessage(serverPlayer, Component.translatable(failureKey));
            }
            return false;
        }
        ...
        if (showStartMessage) {
            MorningKissDialogueService.showMessage(serverPlayer, MorningKissDialogueService.startMessage(
                    "bond.morning_kiss.started", "bond.morning_kiss.started.chat", maid));
        }
        return true;
    }
    ...
    private static void failTask(Iterator<PendingMorningKiss> iterator, PendingMorningKiss task, ServerPlayer player, String failureKey) {
        ...
        if (failureKey != null) {
            MorningKissDialogueService.showMessage(player, Component.translatable(failureKey));
        }
    }
