...
public final class LapPillowHandler {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (!(event.player instanceof ServerPlayer player) || player.level().isClientSide || !LapPillowState.isActive(player)) {
            return;
        }
    ...
    private static void applyEternalUtopia(ServerPlayer player, EntityMaid maid) {
        ...
        MobEffectInstance active = player.getEffect(ModEffects.ETERNAL_UTOPIA.get());
        if (active == null || active.getDuration() < 60) {
            player.addEffect(new MobEffectInstance(ModEffects.ETERNAL_UTOPIA.get(), 220, 0, false, showParticles, true));
            TouhouMaidAffection.LOGGER.info(
                    "[EternalUtopia] Applied/refresh: player={} duration=220",
                    player.getScoreboardName()
            );
        }
        MobEffectInstance maidActive = maid.getEffect(ModEffects.ETERNAL_UTOPIA.get());
        if (maidActive == null || maidActive.getDuration() < 60) {
            maid.addEffect(new MobEffectInstance(ModEffects.ETERNAL_UTOPIA.get(), 220, 0, false, showParticles, true));
        }
    }
    ...
    private static void clearLapPillow(ServerPlayer player, String reason) {
        ...
        player.setForcedPose(null);
        player.removeEffect(ModEffects.ETERNAL_UTOPIA.get());
        ...
        if (maidUuid != null) {
            ...
            if (entity instanceof EntityMaid maid) {
                stopMaidYsm(maid);
                maid.removeEffect(ModEffects.ETERNAL_UTOPIA.get());
                maid.setPose(Pose.STANDING);
