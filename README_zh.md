...
`1.7.5.0+` 起，羁绊页**顶部右侧**（原 TMA AI Hub 按钮槽位）新增「设置」入口，打开的是**独立窗口**面板，左栏四个 tab：**状态 / 功能 / 语音 / 音量**，默认停在**状态**页（只读展示 `/tma morning_kiss status` 的等价内容：开关、语种、缓存策略、缓存统计与「按女仆」列表 + 行内清空）。**功能**页含 7 个功能开关与缓存策略（每池目标条数 / 扫描间隔 / 消费即用）；**语音**页含文本语种、配音语种与可编辑的台词提示词，以及一个打开车万女仆 AI 设置（站点 / 密钥 / 模型 / 音色）的按钮；**音量**页是四项滑块。开关、语种、提示词与缓存策略由服务端权威判定，多人服务器上只有 OP 可以修改（普通玩家只读）；音量是纯客户端设置，拖动即时生效。
...
早安吻数据包可配置静态台词池、亲吻音效行为和 OGG 语音；残血救护数据包可配置救援 OGG 语音与兜底音效。语音池页面可以在保存前试听当前候选语音。完整教程见 [早安吻相关配置说明.md](早安吻相关配置说明.md)，可直接压缩发布的示例包位于 [examples/TMA-Custom-Voice-Pack](examples/TMA-Custom-Voice-Pack)。
...
TMA 不再注册自己的聊天/TTS 站点类型：包括早安吻在内的所有 LLM 与 TTS 请求都走 TLM 自己的站点，因此 API key、模型、音色与启用状态全部由 Touhou Little Maid 自己保存和管理。AI **站点**（地址、密钥、模型、音色）仍由车万女仆自己的 AI 设置管理——「语音」tab 里有一个按钮直接打开它；而早安吻的台词提示词已可在 TMA 面板内直接编辑。
