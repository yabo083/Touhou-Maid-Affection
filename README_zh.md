...
- **K 键亲吻**：亲吻改为使用 **K 键**触发 —— 按一下亲一次，长按也不会重复。原来的潜行 + 右键触发已移除。
- **按住亲吻镜头**：按下 K 镜头推近至女仆脸部；按住期间镜头保持；松开 K 镜头收回。
- **准星精准瞄准**：当女仆渲染高度偏低（坐姿或模型坐姿动画）时，亲吻镜头瞄准你准星指向的位置，坐姿亲吻也能亲到脸。
- **可配置亲吻距离**：`fov.targetMaxDistance`（默认 3 格，原来固定 6 格）。
- **满好感冷却**：3 级好感亲吻冷却默认改为 0.5 秒。
- **羁绊界面提示**：P 点不足时新增说明「使用物品形式的P点解锁」。
...
将准星对准女仆并按 **K 键**即可亲吻 —— 按一下亲一次，长按也不会重复。亲吻会提升好感、播放随机亲吻音效、生成爱心粒子，并触发贴近镜头：按住 K 期间镜头保持在女仆脸部，松开后镜头收回。短时间连续亲吻可触发自定义增益「少女祈祷」。公主抱（鞍）女仆时按 K 键同样可以亲吻。
...
3. 将 `touhou-maid-affection-1.7.3.jar` 放入 `mods` 文件夹。
4. 启动游戏。
