...
### Changed
- 羁绊页（TLM 顶部页签进入的选项卡界面）与它弹出的二级页的**底板改为图片渲染**，与设置面板的图片 GUI 同机制；本次只改底板，按钮 / 下拉框 / 滑块 / 开关 / 数值框 / 列表行 / 全部文字仍由代码绘制。四张可替换贴图都在 `assets/touhou_maid_affection/textures/gui/` 下，按 GUI scale 3 烘焙并 1:1 blit：
  - `bond_page.png`（528×411 = 页面 176×137 × 3）：页面外框与斜面、标题条、表头 / 表尾 / 左右分隔线、内凹内容区；`BondMaidContainerScreen#renderBondPageBackground` 不再自绘任何底色与边框，只保留居中的本地化标题文本。
  - `bond_row.png`（474×69 = 158×23 × 3）：能力行底板，替换原先的纯色 `fill`（行距是 24，但填充盒只有 23 逻辑像素高，贴图必须按 23 烘焙才不会被重采样）。
  - `bond_modal.png`（42×90，九宫格切片 6|2|6 × 22|2|6 逻辑像素）：二级页模态框的外框、面板与表头分隔线；172×150（分栏页）与 166×132（语音页）及未来任意尺寸共用一张图，压暗层仍为代码 `fill`。
  - `bond_inset.png`（24×24，九宫格切片 3|2|3 逻辑像素）：二级页内部的内凹底板，覆盖 `BondScrollableList` 列表框、语音页空列表占位、膝枕页左面板与姿态网格。
  - 四张图当前是**原版风格占位图**（黑描边、上/左亮斜面、下/右暗斜面、内凹槽位），替换同名文件即可换成手绘底板，无需改代码；配色 token、布局、命中测试与滚动逻辑一行未动。
- 新增纯逻辑类 `BondNinePatch`（九宫格切片的源/目标矩形拆分）与贴图入口 `BondGuiArt`，并新增回归测试 `BondNinePatchTest`：断言四角保持烘焙尺寸、中心吸收全部余量、被压扁到小于边框时丢弃空切片而不是产生负矩形。
- 羁绊页签图标重绘为**御币**（杖身向右倾斜、纸垂保持竖直下垂，做出挥动感），并修正其贴图尺寸：图标改为原生 **16×16**（与 TLM 自己的页签图标一致，`MaidTabButton` 是 24×26 按钮、图标按 `x+4, y+6` 画 16×16）。原 `bond_tab_icon.png` 是 256×256 且图案铺满整张，而代码只取左上 16×16，所以游戏里显示的只是整幅图案的一角 —— 现在 `BondMaidGuiTabHandler` 的两个 blit 改用带**显式贴图尺寸**的重载（7 参重载隐含 256×256 图集，用在 16×16 文件上只会取到 1×1 角落），因此以后直接替换一张 16×16 的 PNG 即可生效。
...
- 新增中英 lang 键：`bond.settings.section.cache_policy`、`bond.settings.key.morning_kiss.cache_target_per_pool` / `bond.settings.sub.morning_kiss.cache_target_per_pool`、`bond.settings.key.morning_kiss.cache_scan_interval_ticks` / `bond.settings.sub.morning_kiss.cache_scan_interval_ticks`、`bond.settings.status.pool.general`（通用 / Any）、`bond.settings.key.morning_kiss.immediate_fallback_enabled` 与 `bond.settings.sub.morning_kiss.immediate_fallback_enabled`、`bond.settings.key.morning_kiss.cache_consume_on_use` 与 `bond.settings.sub.morning_kiss.cache_consume_on_use`、`bond.settings.number.tip`，以及状态页恢复只读后重新引用的 `bond.settings.status.language.auto.display` / `bond.settings.status.language.auto.voice`。
- 新增游戏内「全局设置」面板「语音」tab 的「台词提示词」多行编辑框：直接编辑早安吻 AI 台词模板（占位符 `{maid}` 女仆名 / `{player}` 玩家名 / `{pool}` 时段 / `{time}` 允许时段），带占位符图例与「恢复默认」按钮。这是服务端权威的新设置类型 `TEXT`（逻辑键 `morning_kiss.text_prompt`，上限 1024 字符，保存空值写回内置默认模板），沿用现有 OP2 + 审计日志的写入路径。字数计数画在编辑框**内部右下角**（次要文字色），占位符图例独占编辑框下方一整行、占满内容区整宽（完整显示、不再截断），「恢复默认」按钮移到编辑框上方标签行右对齐，「恢复默认」改用面板通用的文字按钮渲染（与「打开」「清空」同一个 `drawTextButton` 模板，无权限时置灰）。编辑框本身支持**框内滚动**：新增子类 `BondPromptBox` 把滚动步长（`scrollRate()` = 9px）与滚动上限（`9 × (总行数 − 可见行数)`）都吸附到文本行网格，编辑框高度取 `9 × 行数 + 4`（49px = 5 行 + 内边距），因此文字永远按整行显示、不会从行中间被切断；鼠标在框内滚轮只滚框内文本、不再带动面板，光标移出可视区时由原版光标监听自动滚动保持可见。
- 新增设置面板「状态」tab：移植 `/tma morning_kiss status` 与 `/tma morning_kiss cache` 的信息，**全部只读**——开关（早安吻 / AI 台词 / AI 语音 / 立即兜底）、语种（`文本语种` / `配音语种`，未配置显示 `auto（跟随游戏语言）` / `auto（跟随女仆 AI 设置）`）、缓存策略（每池目标 / 扫描间隔 / 消费即用）都用纯文本行渲染（没有胶囊开关 / 下拉框 / 数值框 / pending 状态点），缓存统计（条目语音/纯文本、女仆数 / 在途 / 版本号）与按女仆列表（名字 + 各时段池条目数 + 总条目/目标 + 行内「清空」）同样只读。配套新增只读状态通道 `TmaAiStatusRequestPayload` / `TmaAiStatusPayload` 与清缓存操作包 `TmaAiCacheClearPayload`：状态只回该玩家名下的女仆，清缓存需 OP2 + 审计日志并复用 `/tma morning_kiss clear_ai_cache` 完全相同的服务方法。
- 新增设置面板「语音」tab 的「打开车万女仆的 AI 设置」按钮：跳转 TLM 原生 `AIChatSettingsHubScreen`（parent 传本面板，关闭后回到这里），是全局唯一的 AI 入口；TMA 不自建站点表单。
- 新增游戏内「全局设置」面板：在羁绊页顶部右侧（原 TMA AI Hub 按钮的槽位）新增「设置」入口，面板与当前女仆无关，分四个 tab——状态（只读）、功能开关（早安吻 / 女仆主动早安吻 / AI 台词 / AI 语音 / 立即兜底 / 残血救护 / 随机礼物 / 少女祈祷 Buff，末尾是「缓存策略」区：每池目标条数 / 扫描间隔 / 消费即用）、语音（文本语种 / 配音语种下拉框 + 台词提示词 + AI 站点入口）、音量（亲吻音效 / 早安吻语音 / 残血救护 / 语音试听，0.0–1.0 步进 0.05）。无需再手改 toml。
...
### Changed
- 设置面板改为**单张整页底图 + 控件叠加**：外框、表头分隔线、侧栏分隔线、footer 分隔线与左下角写实玫瑰全部烘进 `textures/gui/settings_panel.png`（1020×690 RGBA，已烘入 0.90 不透明度），面板背景改为一次 blit（按面板 340×230 逻辑尺寸在 GUI scale 3 下 1:1，不缩放、不额外着色），代码不再自绘面板底色 / 边框 / 分隔线；屏幕压暗层、tab 高亮、所有控件与配色 token、footer 按钮保持不变。为对齐底图，`BondGuiTokens.MODAL_TITLE_HEIGHT` 24→20（表头分隔线落在面板高的 20/230）、`NAV_WIDTH` 46→50（侧栏分隔线落在面板宽的 50/340），因此内容可视高 168→172px、内容区宽 274→270px（占位符图例 260px / 261px 仍不截断），各 tab 的 `maxScroll` 随可视高自动收敛。
- 「状态」tab「按女仆」列表对同名女仆去歧义：名字列新增纯逻辑助手 `bond/settings/TmaMaidLabels`，输入该玩家的女仆 `(uuid, 名字)` 列表输出显示标签——名字唯一时原样显示（不加噪声），同名出现 ≥2 次时这些行追加 ` #` + UUID 前 4 位十六进制（小写），如 `精灵酒狐 #a1b2`；名字为空/空白回退为可本地化的占位名（新增 lang 键 `bond.settings.status.maid.unknown`，zh `未知女仆` / en `Unknown maid`，由 `TmaSettingsScreen` 传入 `Component.translatable(...).getString()`），并追加短 ID，如 `未知女仆 #a1b2`。名字列仍按原有 `clip(...)` 规则截断到可用宽度，不挤占行内「清空」按钮。配套单测 `TmaMaidLabelsTest`，`TmaSettingsLangKeysTest` 一并守护该新键。
- 设置面板间距放宽（用户反馈「配置页间距太挤」）：`SECTION_HEADER_HEIGHT` 12→15、`ROW_GAP` 3→6、`LANGUAGE_ROW_HEIGHT` 17→20、`PROMPT_BOX_HEIGHT` 36→49（= 9 × 5 行 + 内边距）、`PROMPT_ROW_HEIGHT` 12→14、`STATUS_KV_HEIGHT` 12→13、`STATUS_ROW_GAP` 2→6。上一轮的「语音 tab 打开即免滚动」要求随之作废：功能（294px）/ 语音（208px）/ 状态三个 tab 超出内容可视高 172px（230 模态高 − 20 标题 − 8 内容上留白 − 30 footer）时一律走面板内部滚动。占位符图例仍独占一整行、占满内容区整宽 270px（`340 − 2 − 50 − 8 − 2 − 8`，横向槽位不受间距常量影响），中英文案按原版字宽表复算为 260px / 261px，均 ≤ 270px、不截断。
- 设置面板 tab 顺序改为「状态 / 功能 / 语音 / 音量」：「状态」提到第一位并作为打开面板的默认选中项（`activeTab` 默认 0、`tabLabel`、导航命中测试与文档同步），状态 tab 请求 AI 状态改在 `init()` 里发起；「缓存策略」三个控件从状态页搬到「功能」tab 末尾，状态页对应的三行改回只读纯文本。
- 设置面板尺寸 300×188 → 340×230，侧栏 tab 从 3 个（功能 / 语音 / 音量）扩到 4 个（新增「状态」）；内容超出可视区走内部滚动。配色 token、服务端权威语义与既有状态机不变。
...
- 全局羁绊 UI 配色统一为「暖木底 + 玫瑰（交互面 / 装饰）+ 金（仅高亮文字）」：`BondGuiTokens` 换用新调色板（保留原常量名，新增开关 / 字段 / 滑块 / 导航等语义色常量），所有调用点自动跟随。设置面板同步按设计稿收口：300×188 模态框、46px 侧栏三 tab、胶囊开关、78×20 字段式下拉、88×13 滑块（数值金色居中）、分区作用域标签、行内请求状态点与「完成 / 重载」footer。不影响功能与其它二级页布局尺寸（仍为 172×150）。
...
### Removed
- 删除独立玫瑰贴图 `textures/gui/rose_vine.png` 及其绘制代码：`TmaSettingsScreen#renderVine`、`ROSE_VINE` 资源常量、`NAV_VINE_WIDTH` / `NAV_VINE_HEIGHT` / `NAV_VINE_TEXTURE_WIDTH` / `NAV_VINE_TEXTURE_HEIGHT` 尺寸常量与相关 javadoc 一并移除（全仓 `grep -ri rose_vine` 零命中），玫瑰改由整页底图承载。
