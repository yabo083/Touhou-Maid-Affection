...
### Added
- 新增 `fov.targetMaxDistance` 配置：K 键亲吻的最大触发距离，默认 3 格（原为固定 6 格）。
- 新增 `fov.cameraSittingFaceOffsetY` 配置：坐姿/低姿态渲染女仆亲吻瞄准的垂直微调。
- 羁绊界面在 P 点不足时新增提示「使用物品形式的P点解锁」。

### Changed
- 亲吻触发改为仅 K 键（移除潜行 + 右键），并改为按键边沿触发：按一下亲一次，长按也只亲一次。
- 亲吻镜头改为按键控制：按下 K 镜头推近至女仆脸部，按住期间镜头保持，松开 K 镜头收回。
- 亲吻瞄准跟随准星：当女仆渲染高度偏低（坐姿/模型坐姿动画）时，亲吻目标自动落在准星指向的位置。
- 3 级好感亲吻冷却默认从 0 秒改为 0.5 秒（10 tick）。

### Removed
- 移除潜行 + 右键亲吻触发方式及 CarryOn 右键冲突规避逻辑。
- 移除 `fov.holdTicks` 配置（镜头停留时间改由按键控制）。

### Fixed
- 修复长按 K 键时亲吻镜头反复抽搐的问题（按键自动重复导致亲吻动画反复重启）。
